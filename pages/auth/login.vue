<template>
  <div>
    <Loading />
    <v-card class="mx-auto text-center flat my-12" maxWidth="384" color="#fff">
      <v-card-title class="bg-color pa-0">
        <p class="text-center mx-auto pt-4 font-weight-black">ログイン</p>
      </v-card-title>
      <v-card-text>
        <v-form>
          <v-text-field v-model="email" prependIcon="mdi-email" label="メールアドレス" />
          <v-text-field
            v-model="password"
          ...
          <nuxt-link to="/auth/ResetPassword" class="d-block link"
            >パスワードお忘れの方はこちら</nuxt-link
          >
        </v-form>
      </v-card-text>
      ...
      <v-form class="my-4">
        ...
        >
          <span>
            <v-img height="20px" width="20px" :src="image_src_google" class="mr-4" />
          </span>
          Googleアカウントでログイン
        </v-btn>
      </v-form>
